...
```ts
const getsFreeShipping = (cart: Cart) => { // 주문하는 제품이 담긴 카트를 넘김
  return calcTotal(cart) >= 20; // 카트에서 값을 모두 더해주는 calcTotal을 사용
}

const calcTotal = (cart: Cart) => { // 현준님 코드 좀 쓰겠습니다 😆
...
```
기존에 **장바구니에 담긴 상품이랑 주문할 상품의 합으로 무료배송 여부**를 보여주던 함수에서 **카트 넘기면 무료배송 여부를 반환**하도록 수정
...
const addItem = (cart: Cart, name: string, price: number) => {
  ...
  newCart.push({ // 2. item 객체를 생성, 3. 복사본에 item 추가
    name,
    price
  });
...
```ts
const makeCartItem = (name: string, price: number) => { // item 구조만 알고 있는 함수
  return {
    name,
    price
  }
...
const addItem = (cart: Cart, item: Item) => { // cart 구조만 알고 있는 함수
